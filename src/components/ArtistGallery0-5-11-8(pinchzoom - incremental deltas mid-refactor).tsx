...
export const ArtistGallery: React.FC<ArtistGalleryProps> = ({ images = [] }) => {
    ...
    const handlePointerMove = (e: React.PointerEvent) => {
        ...
        // new isPinching block (incremental deltas)
            ...
            const nextPanXUnclamped = panXRef.current + originDeltaX;
            const nextPanYUnclamped = panYRef.current + originDeltaY;

            // Commit
            zoomScaleRef.current = nextZoomUnclamped; // why are these so necessary, here, for incremental?
            pendingRef.current.zoomScale = nextZoomUnclamped;

            panXRef.current = nextPanXUnclamped; // why are these so necessary, here, for incremental?
            panYRef.current = nextPanYUnclamped; // why are these so necessary, here, for incremental?
            pendingRef.current.panX = nextPanXUnclamped;
            pendingRef.current.panY = nextPanYUnclamped;
            pinchPrevDistanceRef.current = curDist;

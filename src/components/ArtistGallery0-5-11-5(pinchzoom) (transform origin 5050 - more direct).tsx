    ...
    const handlePointerMove = (e: React.PointerEvent) => {
        ...
        if (noLongerPinchingRef.current) {
            ...
            if (pinchStartDistanceRef.current === null) {
                ...
                //
                // Let:
                //   C0 = (centerStartX, centerStartY)  = image center on screen at pinch start
                ...
                //   => C1 = P - z1 * p_anchor
                //
                // We don't store C1 directly as state; we still store panX/panY.
                ...
                //   L0 = C0 - panStart
                //
                // For the new center C1:
                //   panNext = C1 - L0 = panStart + (C1 - C0)
                //
                // So we store:
                //   - C0 in pinchOriginRef (repurposed as "startCenter")
                //   - panStart in pinchBaseTopLeftRef (repurposed as "startPan")
                //   - p_anchor in pinchAnchorRef (anchor in image-centered coords)
                //-----------------------------------------------------------------

                // Image center on screen at pinch start
                const centerStartX = imgRect.left + imgRect.width / 2;
                const centerStartY = imgRect.top + imgRect.height / 2;

                // Store center-at-start (C0)
                pinchOriginRef.current = { x: centerStartX, y: centerStartY };

                // Store pan-at-start (panStart)
                pinchStartPanRef.current = { x: pinchStartPanX, y: pinchStartPanY };

                // Anchor in image-centered local coords:
                // p_anchor = (P0 - C0) / z0
                const anchorX =
                    (pinchCenterX - centerStartX) / pinchStartBaseZoom;
                const anchorY =
                    (pinchCenterY - centerStartY) / pinchStartBaseZoom;
                ...
                const startCenter = pinchOriginRef.current;      // C0
                const startPan = pinchStartPanRef.current;    // panStart
                ...
                //   C1 = P - z1 * p_anchor
                const nextCenterX =
                    pinchCenterX - nextZoomUnclamped * anchor.x;
                const nextCenterY =
                    pinchCenterY - nextZoomUnclamped * anchor.y;

                // panNext = panStart + (C1 - C0)
                const deltaCenterX = nextCenterX - startCenter.x;
                const deltaCenterY = nextCenterY - startCenter.y;

                const nextPanXUnclamped = startPan.x + deltaCenterX;
                const nextPanYUnclamped = startPan.y + deltaCenterY;

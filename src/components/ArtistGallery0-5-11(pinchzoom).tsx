...
export const ArtistGallery: React.FC<ArtistGalleryProps> = ({ images = [] }) => {
    ...
    const pinchCenterRef = useRef<{ screenX: number; screenY: number } | null>(null);
    const pinchAnchorX = useRef(1);
    const pinchAnchorY = useRef(1);
    ...
    const handlePointerDown = (e: React.PointerEvent) => {
        ...
        // const startCenterX = (ev1.clientX + ev2.clientX) / 2;
        // const startCenterY = (ev1.clientY + ev2.clientY) / 2;
        ...
        //     startDistance,
        //     startScale: zoomRef.current,        // your current zoom
        //     // Where, in image-local coords, is the pinch center?
        //     anchorX: (startCenterX - rect.left) / zoomRef.current,
        //     anchorY: (startCenterY - rect.top) / zoomRef.current,
        // };
    };
    ...
    const handlePointerMove = (e: React.PointerEvent) => {
        ...
        // If two pointers are down, check for pinch gestures
            ...
            // Midpoint of the two touches in viewport coordinates
            const centerX = (ev1.clientX + ev2.clientX) / 2;
            const centerY = (ev1.clientY + ev2.clientY) / 2;

            // First pinch frame – initialize baseline
            if (pinchStartDistanceRef.current === null) {
                pinchStartDistanceRef.current = curDiff;
                pinchStartZoomRef.current = zoomRef.current;

                // Capture pan at pinch start
                panXStartRef.current = panXRef.current;
                panYStartRef.current = panYRef.current;

                // Find where the pinch center is relative to the image center
                ...
                if (img) {
                    ...
                    // Vector from image center to pinch center (in screen px)
                    const screenX = centerX - imgCenterX;
                    const screenY = centerY - imgCenterY;

                    pinchCenterRef.current = { screenX, screenY };
                    pinchAnchorX.current = (imgCenterX) / zoomRef.current;
                    pinchAnchorY.current = (imgCenterY) / zoomRef.current;
                    ///
                    pinchAnchorX.current = centerX - rect.left;
                    pinchAnchorY.current = centerY - rect.top;

                } else {
                    pinchCenterRef.current = { screenX: 0, screenY: 0 };
                }

                return;
            }

            // From here on, we have baseline data
            const startDist = pinchStartDistanceRef.current;
            const baseZoom = pinchStartZoomRef.current;
            if (!startDist || !baseZoom || !pinchCenterRef.current) return;

            const { screenX, screenY } = pinchCenterRef.current;
            const p0x = panXStartRef.current;
            const p0y = panYStartRef.current;

            // Proposed new zoom
            let nextZoomRaw = (curDiff / startDist) * baseZoom;
            // Clamp it
            nextZoomRaw = clamp(nextZoomRaw, MIN_ZOOM, MAX_ZOOM);

            // Keep the pinch center visually fixed:
            // pan1 = screen * (1 - z1/z0) + pan0 * (z1/z0)
            const ratio = nextZoomRaw / baseZoom;
            const newPanX = screenX * (1 - ratio) + p0x * ratio;
            const newPanY = screenY * (1 - ratio) + p0y * ratio;
            ...
            const newTranslateX = centerX - pinchAnchorX.current * nextZoomRaw;
            const newTranslateY = centerY - pinchAnchorY.current * nextZoomRaw;

            setTranslateX(newTranslateX);
            setTranslateY(newTranslateY);
            console.log("The transform origin is:", getComputedStyle(imageRef!.current as HTMLElement).transformOrigin);

            zoomRef.current = nextZoomRaw;
            setZoom(nextZoomRaw);
        }
    };
    ...
    const handlePointerUp = (e: React.PointerEvent) => {
        ...
        if (isPinchGesture) {
            ...
            // If we zoomed back to 1, recenter / reset pan
            if (zoomRef.current <= 1) {
    ...
    useLayoutEffect(() => {
        ...
        // If we zoomed back to 1, recentre / reset pan
        // if (zoomRef.current <= 1) {
        //     maxPanXRef.current = 0;
        //     maxPanYRef.current = 0;
        //     panXRef.current = 0;
                            ...
                            >
                                ...
                                <div className="flex items-center justify-center w-screen">
                                    <img
                                        ...
                                        className="max-h-[80vh] w-auto max-w-full object-contain shadow-lg bg-black/20"
                                        style={{
                                            transform: `translate(${panX + translateX}px, ${panY + translateY}px) scale(${exitScale * zoom})`,
                                            transformOrigin: `${pinchAnchorX.current}px ${pinchAnchorY.current}px`,
                                            opacity: imageOpacity,
                                            transition: draggingRef.current
                                                ? "none"
